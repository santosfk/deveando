...
export const RegisterModal = ({setModalIsOpen}: Props) => {
  ...
  <span>Quando um novo post você será notificado no email : {session?.user?.email}</span>
    </LoggedContainer>:
    <Container>
      <Title>Faça login Aqui!!</Title>
      <span>{}</span>
      <span>dev utiliza o github</span>
        <Button color='#3CB57A' onClick={()=>signIn('github')}>Logar <GithubLogo size={22} weight="fill" /> </Button>
        <Button color='#D61E2D' onClick={handleCloseModal}>fechar</Button>       
    </Container>
    }
    </>
  )
